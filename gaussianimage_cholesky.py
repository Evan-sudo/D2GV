...
class GaussianImage_Cholesky(nn.Module):
    def __init__(self, loss_type="L2", **kwargs):
        ...
        self.features_dc_quantizer = FakeQuantizationHalf.apply
        self.cholesky_quantizer = FakeQuantizationHalf.apply
    ...
    def forward_dynamic_test(self, deformed_xyz, deformed_opacity, deformed_features):
        xys, depths, self.radii, conics, num_tiles_hit = project_gaussians_2d(torch.tanh(self._xyz+deformed_xyz), self._cholesky + self.cholesky_bound, self.H, self.W, self.tile_bounds)
        out_img = rasterize_gaussians_sum(xys, depths, self.radii, conics, num_tiles_hit,
                deformed_features, self.get_opacity, self.H, self.W, self.BLOCK_H, self.BLOCK_W, background=self.background, return_alpha=False)
        # out_img = rasterize_gaussians_sum(xys, depths, self.radii, conics, num_tiles_hit,
    ...
    def forward_quantize(self, deformed_xyz, deformed_opacity, deformed_features):
        means = torch.tanh(self._xyz+deformed_xyz)
        cholesky_elements = self.cholesky_quantizer(self._cholesky)
        cholesky_elements = cholesky_elements + self.cholesky_bound
        colors = self.features_dc_quantizer(self.get_features)
        opacity = self.features_dc_quantizer(self._opacity)
        ...
        # out_img = rasterize_gaussians_sum(self.xys, depths, self.radii, conics, num_tiles_hit,
        #         colors+deformed_features, self._opacity+deformed_opacity, self.H, self.W, self.BLOCK_H, self.BLOCK_W, background=self.background, return_alpha=False)
        out_img = rasterize_gaussians_sum(self.xys, depths, self.radii, conics, num_tiles_hit,
