...
class GaussianImage_RS(nn.Module):
    ...
    def forward_dynamic(self, deformed_xyz, deformed_rot, deformed_scale, deformed_features):
        xys, depths, self.radii, conics, num_tiles_hit = project_gaussians_2d_scale_rot(torch.tanh(self._xyz+deformed_xyz), torch.abs(self._scaling+deformed_scale+self.bound), self.rotation_activation(self._rotation+deformed_rot)*2*math.pi, self.H, self.W, self.tile_bounds)
        out_img = rasterize_gaussians_sum(self.xys, depths, self.radii, conics, num_tiles_hit,
                self.get_features, self.get_opacity, self.H, self.W, self.BLOCK_H, self.BLOCK_W, background=self.background, return_alpha=False)
        out_img = torch.clamp(out_img, 0, 1) #[H, W, 3]
        out_img = out_img.view(-1, self.H, self.W, 3).permute(0, 3, 1, 2).contiguous()
