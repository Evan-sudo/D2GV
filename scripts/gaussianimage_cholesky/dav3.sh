...
# 创建日志文件
log_file="training_log_swan.txt"
echo "Training Log - $(date)" > "$log_file"

find "$data_path/swan" -mindepth 1 -maxdepth 1 -type d -name "swan*" | sort | while read -r segment; do
    ...
    for num_points in 40000; do
        ...
        CUDA_VISIBLE_DEVICES=6 python train_deform.py -d "$segment" \
            --data_name UVG2/$(basename "$segment") --model_name GaussianImage_Cholesky --num_points "$num_points" --iterations 80000 \
            2>&1 | tee "$temp_log"
