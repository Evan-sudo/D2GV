...
# 创建日志文件
log_file="training_log_joc.txt"
echo "Training Log - $(date)" > "$log_file"

# 按字母顺序遍历 Beauty 下所有 segment 文件夹，例如 Beauty_1, Beauty_2, ...
find "$data_path/Jockey" -mindepth 1 -maxdepth 1 -type d -name "Jockey*" | sort | while read -r segment; do
    ...
    for num_points in 40000; do
        ...
        CUDA_VISIBLE_DEVICES=4 python train_deform.py -d "$segment" \
            --data_name UVG/$(basename "$segment") --model_name GaussianImage_Cholesky --num_points "$num_points" --iterations 140000 \
            2>&1 | tee "$temp_log"
